...
class Member:
    ...
    def __init__(self, name, id_number, borrowed_books):
        ...
        print(
            f"the member infos are: name: {name} , id_number:{id_number} and borrowed books are: {borrowed_books}  "
        )
...
class Library:
    ...
    def __init__(self):
        self.books = {Book.book_info}
        self.members = {}

    def add_book(self, titre, writer, disponible):

        for book in self.books:
            book.update(titre, writer, disponible)

        return print(f"book : {book} was added.")

    def remove_book(self, titre, disponible):
        try:
            for book in self.books:
                if titre == titre and disponible:
                    self.books.remove(self, titre, disponible)

            print("book was removed! ")

        except Exception:
            print("the book did not exists! so nothing changed")

    def list_books(self):
        return self.books

    def add_member(self, name, id_number, borrowed_books):
        Member.members.update(name, id_number, borrowed_books)
        return print(f"added member: {name}")

    def remove_member(self, name, id_number, borrowed_books):
        Member.members.pop(name)
        return print(f"deleted member: {name}")

    def list_members():
        return Member.all_members()


def borrow_book(book_name, disponibility):
    try:
        book_name = input("write which book you want? ")
        if book_name == Book.name:
            if disponibility == "yes":
                print("you can take it... ")
                disponibility == "no"
            else:
                print("it is not disponible...")
        else:
            print("the book does not exist!")
    except NameError:
        print("did not found the book!")


def return_book(book_name, disponibility):
    try:
        book_name = input("write which book you return? ")
        if book_name == Book.name:
            if disponibility == "no":
                print("you can give it back... ")
                disponibility == "yes"
            else:
